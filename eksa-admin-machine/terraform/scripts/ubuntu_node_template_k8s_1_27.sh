...
read -p 'Final name for EKS-A ubuntu node template, e.g. ubuntu-2004-kube-v1.27: ' ubuntu_template_name
read -p 'vSphere Resource pool name, e.g. Test: ' vsphere_resource_pool
sudo -Hiu saith rm -rf eks-anywhere-downloads
sudo -Hiu saith rm -rf eks-anywhere-downloads.tar.gz
sudo -Hiu saith eksctl anywhere download artifacts
sudo -Hiu saith tar -xzf eks-anywhere-downloads.tar.gz
file=$(sudo -Hiu saith ls /home/saith/eks-anywhere-downloads/1.27/eks-distro/ | grep kubernetes)
...
image-builder build --os ubuntu --hypervisor vsphere --release-channel 1-27 --vsphere-config $HOME/vsphere-connection.json
